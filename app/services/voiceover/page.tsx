...
// Static page for the Voiceover service (non-null assertion since it's in data)
const service = servicesData.find(s => s.id === "voiceover")!
...
export default function VoiceoverPage() {
  ...
  // Service is defined in data; avoid early return to keep hooks order valid
  ...
  // Load voiceover audio files from new/voiceover/voiceover via API
  const [audioFiles, setAudioFiles] = useState<string[]>([])
  
  // Mapping for correct audio file names based on index
  const audioFileNames = useMemo(() => {
    if (isRTL) {
      return [
        "ibrand marketing",
        "مكتب القائد للسفريات",
        "شركة أبو سعد للتصدير والنقل الدولي",
        "شركة أبو سعد للتصدير والنقل الدولي",
        "عيادة فيكتور دنت",
        "الحبشي للأثاث"
      ]
      ...
      return [
        "ibrand marketing",
        "Al Qaed Travel Office",
        "Abu Saad Export & International Transport",
        "Abu Saad Export & International Transport",
        "Ayad Vector Dent Clinic",
        "El-Hobashy Furniture"
      ]
    }
  }, [isRTL])
  ...
  // Simple elegant audio list UI
  const AudioList = () => {
    const [current, setCurrent] = useState<number | null>(null)
    const [isPlaying, setIsPlaying] = useState<boolean>(false)
    const audioRef = useRef<HTMLAudioElement | null>(null)
    // خلفية بديلة عند عدم توفر بوستر للصوت

    // Load and autoplay when current changes
    useEffect(() => {
      const audio = audioRef.current
      if (!audio) return
      if (current == null) {
        // If deselected, ensure stopped
        audio.pause()
    ...
    return (
      <div className={`mt-8 rounded-2xl p-6 ${isDark ? "bg-slate-800" : "bg-white"} border ${isDark ? "border-slate-700" : "border-gray-200"}`}>
        <h3 className={`text-2xl font-bold mb-4 ${isDark ? "text-white" : "text-slate-900"}`}>
          {isRTL ? "عينات التعليق الصوتي" : "Voiceover Samples"}
        </h3>
        <div className="space-y-3">
          {audioFiles.length === 0 && (
            <div className={`${isDark ? "text-gray-300" : "text-gray-600"}`}>
              {isRTL ? "لا توجد ملفات صوتية متاحة الآن" : "No audio files available yet"}
            </div>
          )}
          {audioFiles.map((url, idx) => {
            const active = current === idx
            const fileName = audioFileNames[idx] || (isRTL ? `عينة رقم ${idx + 1}` : `Sample ${idx + 1}`)
            return (
              <div key={idx} className={`flex items-center justify-between rounded-xl px-4 py-3 border ${isDark ? "border-slate-700" : "border-gray-200"} ${active ? (isDark ? "bg-blue-600/10" : "bg-blue-50") : ""}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    className={`${isDark ? "bg-blue-600 hover:bg-blue-700" : "bg-blue-600 hover:bg-blue-700"} text-white px-4 py-2 rounded-lg`}
                  >
                    {active ? (isPlaying ? (isRTL ? "إيقاف مؤقت" : "Pause") : (isRTL ? "تشغيل" : "Play")) : (isRTL ? "تشغيل" : "Play")}
                  </Button>
                  {/* Stop */}
                  <Button
                    variant="outline"
                    ...
                    className={`px-4 py-2 rounded-lg ${isDark ? "border-slate-600 text-blue-300 hover:bg-slate-700" : "border-blue-300 text-blue-700 hover:bg-blue-50"}`}
                    disabled={!active}
                  >
                    {isRTL ? "توقيف" : "Stop"}
                  </Button>
  ...
  return (
    <div dir={isRTL ? "rtl" : "ltr"} lang={isRTL ? "ar" : "en"} className={`min-h-screen overflow-x-hidden ${isDark ? "bg-slate-900" : "bg-gray-50"}`}>
      ...
      {/* Results Section (auto from new/voiceover + gallery) */}
      <section className={`py-20 ${isDark ? "bg-gradient-to-br from-slate-900 to-slate-800" : "bg-gradient-to-br from-blue-50/30 to-white"}`}>
        <div className="container mx-auto px-4">
          ...
          {/* First audio display (gallery) disabled per request */}
          {/** <ServiceResultsGallery serviceId={service.id} showBadge={false} /> **/}
          <AudioList />
        </div>
      </section>

      {/* Our Previous Work Section (reuse homepage portfolio) */}
      <section className={`${isDark ? "bg-slate-900" : "bg-gradient-to-br from-blue-50 to-white"}`}>
        <div className="container mx-auto px-4 py-20">
          ...
          >
            <Portfolio selectedIds={selectedPortfolioIds} compact />
          </motion.div>
        </div>
      </section>
